...
Used to specify widget width. Default value (if `width` is not specified) is `auto`, which means that widget width will be determined
by the widget contents. Other supported values are: `small` (25% of the screen width), `medium` (50%), `large` (75%) and `full` (100%).

TIP: We don't recommend using auto-width as this may cause widget contents to overflow onto the widget to the right of it.

* `height`

Used to specify widget height. Default value (if `height` is not specified) is `auto`, which means that widget height will be determined
by the widget contents. Other supported values are: `small` (25% of the screen height), `medium` (50%), `large` (75%) and `full` (100%).

TIP: We don't recommend using auto-height as this may cause widget contents to overflow onto the widget below.
...
.Custom styling, position 0, display name in the header, width 25%, height auto:
[source,xml]
----
...
.Default styling, position 1, display name in the header, width 50%, height 50%:
[source,xml]
----
<widget xmlns="urn:enonic:xp:model:1.0">
  ...
  <config>
    <property name="width" value="medium"/>
    <property name="height" value="medium"/>
