...
Our goal is to enable any developer - PHP, .net, Java, Python, etc to quickly be productive with Enonic XP.
...
JavaScript controllers make out the core concept of the Enonic XP framework.
Every javascript file is considered a controller, and the controllers are initiated through their exports.

For instance, every application may have a <<Application Controller>> that is invoked based on application life-cycle events.
...
When application lifecycle events like start or stop occurs, it can trigger some code. If you add a ``main.js`` file to the root of your application
