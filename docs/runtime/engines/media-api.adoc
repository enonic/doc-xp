...
. Attachment Locator with optional draft branch and optional fingerprint +
  `<media-project>(:<media-branch>)/<content-id>(:<attachment-fingerprint>)/<attachment-name>` +
  ...
  * fingerprint is not required. If not supplied the response will not contain "immutable" `Cache-Control` header.

. Optional download parameter +
  `?download` - the response will contain `Content-Disposition` header, if specified.

Example:
  `/site/site-project/draft/mysite/_/media:attachment/media-project:draft/c82262a6-77d6-4276-b0e7-fe82f4eb57b8:ec25d6e4126c7064f82aaab8b34693fc/attachment.png?download`

Usually, for Site Engine, `<project>` and `<media-project>` are the same as well `<branch>` and optional `<media-branch>`. This is the default behavior and by default other options are prohibited. This can be configured with `endpoint.media.scope = <project1:draft>, <project2>` Virtual Host Context.
...
TODO. This is a copy from legacy Image Service. URL formats are invalid.
...
. Endpoint, followed by a forward slash. +
  Format: `:8080/site/<project>/<branch>/<path-to-site>/_/media:image/` +
  On a live site behind virtual host, everything before the path is typically hidden.
