...
NOTE: The first snapshot only stores markers in the repository for the current state.
...
Export data from every repository.
...
|`sourceRepoPath`
|String
|path of data to export.
Format: `<repo-name>:<branch-name>:<node-path>` e.g. `cms-repo:draft:/some-content`
...
|`xslParams`
|JSON
|parameters to pass to the XSL transformations before importing nodes.
Format: `{"applicationId": "com.enonic.myapp"}`
...
This option could for example be used for renaming types or fields.
The .xsl file must be located in the `$XP_HOME/data/export` directory.
====
...
|`sha512`
|String
|image:xp-760.svg[XP 7.6.0,opts=inline] application file SHA-512 checksum.
Optional.
If provided, and checksum does not match, installation will fail.
|===
...
image:xp-760.svg[XP 7.6.0,opts=inline] The client initiates the SSE connection by using the media type `text/event-stream` in the `Accept` header.
Then it gets updates automatically without requesting the server.
...
Only content of a media type (super-type = _base:media_) are processed.
...
|`sourceBranchPath`
|String
|target content path to be reprocessed.
Format: `<branch-name>:<content-path>`. e.g `draft:/`
...
This is usually required after upgrades and may be useful in many other cases.
...
NOTE: Task info can be fetched with <<#_task, task>> request.
...
The format based on the ISO-8601 duration format PnDTnHnMn.nS with days considered to be exactly 24 hours.
|===
