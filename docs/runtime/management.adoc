...
|Boolean
|outputs dump output to an archive (`%name%`.zip) file
|===

NOTE: In XP 8, dumps are always saved as ZIP archives (`{name}.zip`), regardless of the `archive` parameter.
...
|`archive` image:xp-730.svg[XP 7.3.0,opts=inline]
|Boolean
|set to `true` for XP 8 dumps (ZIP archives); set to `false` for XP 7 dumps (directory format)
|===
