...
Applications ready for XP 8:: All applications - 3rd party and custom-built - must be available in XP 8 compatible versions *before* you begin the migration. The migration cannot complete without them.

IMPORTANT: Updating custom applications is a developer task that must be done ahead of time. Follow the https://developer.enonic.com/docs/code/stable/upgrade[application upgrade instructions] in the Enonic Dev kit documentation, and confirm with the maintainers of any 3rd party apps that XP 8 versions are available.
...
===== WebSocket same-origin check

WebSocket handshakes are now subject to a built-in same-origin check.
If a WebSocket controller does not supply its own `checkOrigin` function, handshake requests with an `Origin` that does not match the request host are rejected.
In XP 7 such requests were accepted from any origin.

If your applications rely on cross-origin WebSocket connections, implement a `checkOrigin` function in the WebSocket controller to explicitly allow trusted origins.

If legitimate same-origin connections are rejected after the upgrade, the usual cause is a reverse proxy that does not propagate the public scheme/host/port to XP — fix the proxy configuration.
As a last resort, the check can be disabled with the `websocket.defaultOriginCheck` setting in the <<../config/system#jetty,Jetty configuration>>.
...
Generate (or download, if already exists) a private key for a service account with Administrator role, one on the existing XP7 instance (to be used for `dump create`), another on the new XP8 instance (to be used for `dump load`). The key will be automatically downloaded to your disk after it's generated. Read <<../security/service-accounts#, here>> to learn how to generate keys for service accounts.
...
IMPORTANT: The `--archive` parameter must be provided, otherwise XP8 will not be able to read the dump. The migration dump must also include version history and all binaries (attachments) — both are included by default; do not pass `--skip-versions`.
