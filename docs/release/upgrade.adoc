...
* When content is created `image/webp` and `image/avif` media types create `media:image` content type, instead of `media:unknown`. Old versions of Content Studio might not work correctly with WebP and AVIF media types. Upgrae t the latest version of Content Studio.
...
* Andy vacuuming that affects blobs will delete snapshots after the vacuuming is done. <<deployment/backup-restore#vacuum, Vacuuming>> documentation has been updated to reflect this change.
