...
* Applications can now be installed on Project level. This makes Project root similar to Site: any content provided by apps can be created on the Project root and Site <<cms/mappings#_mappings, Mappings>> provided by apps allow intercepting content calls.
* You need to reconfigure All Project level Applications configured before version 7.12, if you have any. This is not a breaking change, as this feature was not announced in 7.11 and was not supported.
* Content API is now enforced to support only `draft` and `master` branch. Node API continues to support custom branches.
* Content API does not allow to create/modify/delete content directly on `master` branch. All content must be published from `draft` to `master.`
...
* Node state has been deprecated and no longer in use. In was only used for content's `PENDING_DELETE` status.
...
* `draft` branch access is now restricted. By default, only `system.admin.login` role has access. Additional principles can be configured in <<deployment/config#_portal, portal config>> by `draftBranchAllowedFor` setting. Note that `system.admin` role always has access.
* Bundled GraalVM has been updated to version 22.3. Other JVM versions are not supported, although they may also work.
