...
This procedure applies to feature (7.1.0, 7.2.0, 7.3.0, 7.4.0) and fix upgrades (7.2.4, 7.3.1, etc).
...
=== Clusters
Clusters can be upgraded without downtime.
...
* Create the configuration file `com.enonic.xp.hazelcast.cfg` and consider the values in it as described in
   <<../deployment/config#hazelcast, com.enonic.xp.hazelcast.cfg>> .
* Open port 5701 between the nodes in the network.  This port is used for all internal communication in the cluster.
...
but if you have a set-up with dedicated master and data nodes, the minimum should be set to one more than the number of data nodes,
so that at least one master node is required.  The cluster will neither elect a master node nor start XP message communications
until the required number of nodes in the cluster has been reached.
