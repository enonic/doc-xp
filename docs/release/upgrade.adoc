...
It is possible to configure the header via `page.defaultContentSecurityPolicy` value or go back to previous behavior by setting `page.defaultContentSecurityPolicy` to an empty value.
...
* Security headers are now specified in <<deployment/config#_header_filter, header filter config>> be default.
It may prevent some pages to work properly if they, for instance, require to be loaded from IFrame. You can customize default headers
in <<deployment/config#_header_filter, header filter config>> `headerConfig` value to get back to previous behaviour.
