...
* The image service now generates progressive JPEG images by default. This can be changed back in <<../config/system#image>> by setting `progressiveJpeg` to empty string.

* GZIP HTTP compression is disabled by default. It can be enabled in <<../config/system#jetty, Jetty config>>. By setting `gzip.enabled = true`.
...
* <<../config/system#repo, Repo config>> `cache.size` parameter no longer works. Use `cache.capacity` instead.
...
IMPORTANT: In `7.13.1` you can bring back the old deprecated behaviour making Content API inject empty regions by setting `resolveEmptyRegions` parameter to `true` in the <<../config/system#content_config, content config>>
...
* Content API audit-log no longer logs `system.content.update` events by default. This can be changed back by setting `auditlog.filter = *` in <<../config/system#content_config, content config>>.
* `draft` branch is now restricted for unauthenticated access. By default, only `system.admin.login` role has access. Additional principles can be configured in <<../config/system#portal, portal config>> via `draftBranchAllowedFor` setting. Note that `system.admin` role always has access, regardless of the config.
...
To prevent virtual applications from overriding schemas use <<../config/system#application, application config>> `virtual.schema.override = false`
...
* <<../config/system#portal, Portal config>> now specifies default `Content-Security-Policy` for attachments and images.
...
* <<../config/system#admin, Admin config>> now specifies default Content-Security-Policy for page `preview` and `inline` mode (used by Content Studio).
...
* Certain Security headers are now specified in <<../config/system#header_filter, header filter config>> by default.
...
You can customize default headers in <<../config/system#header_filter, header filter config>> `headerConfig` value to get back to previous behaviour.
...
* Image Service now has memory usage constraints  configured by `memoryLimit` parameter in <<../config/system#image, Configuration Files / Image>>.
...
You can revert to previous behavior by setting `session.cookieSameSite =` in <<../config/system#jetty, Configuration Files / Jetty>>
...
* Image Service now limits maximum image size and number of filters applied. You can configure thresholds in <<../config/system#image, Configuration Files / Image>>
...
You can revert to previous behavior by setting `attachments.allowUnsafeNames = true` in <<../config/system#content, Configuration Files / Content>>
...
You can change default settings in <<../config/system#portal, Configuration Files / Portal>>
...
You can switch back to legacy mode by setting `contentTypePatternMode = LEGACY` in  <<../config/system#admin_rest, Configuration Files / Admin>> for `allow-content-type` and `allow-content-types`, but it won't affect `allow-child-content-type` pattern matching.
...
* XP does not proxy market.enonic.com requests anymore, meaning that browser should have access to https://market.enonic.com. URL can be changed in <<../config/system#market, Configuration Files / Market>>.
...
* Create the configuration files `com.enonic.xp.hazelcast.cfg` and `com.enonic.xp.web.sessionstore.cfg` and consider the values in them as described in
   <<../config/system#hazelcast, Configuration Files / Hazelcast>> and <<../config/system#sessionstore, Configuration Files / SessionStore>>.
