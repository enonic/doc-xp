...
The lowercase method names (`get`, `post`, `delete`, `put`, `patch`) are deprecated but still supported for backward compatibility. It is recommended to migrate to the new uppercase names (`GET`, `POST`, `DELETE`, `PUT`, `PATCH`). 
...
* Avoid conflicts with JavaScript reserved words like `delete`
* Prevent naming conflicts when using ES6 module imports (e.g., `import {get} from '/lib/xp/content'`)
