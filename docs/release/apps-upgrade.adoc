...
The lowercase method names (`get`, `post`, `delete`, `put`, `patch`, `all`) are deprecated but still supported for backward compatibility. It is recommended to migrate to the new uppercase names (`GET`, `POST`, `DELETE`, `PUT`, `PATCH`, `ALL`).
