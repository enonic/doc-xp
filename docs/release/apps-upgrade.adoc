...
We recommend bundling Gradle wrapper of version 9.2.1 or later in your project.
...
plugins {
    id 'java'
    id 'maven-publish'
    id 'com.enonic.xp.base' version '4.0.0-A1'
}
...
New optional `batchSize` parameter is added to control the number of nodes exported in each batch (default: `1000`).
...
Folder-based exports from XP 7 must be converted to a ZIP archive before they can be imported in XP 8.
...
New optional `nodeSkipped` callback parameter is added, called during import with the number of nodes skipped since the last call.

The result object now includes a `skippedNodes` array alongside the existing `addedNodes`, `updatedNodes`, etc.

The `source` path must now point to a ZIP archive (`.zip` file).
Folder-based exports from XP 7 must be converted to ZIP format before importing.

XP 7
[source,typescript]
----
const result = importNodes({
    source: resolve('/import/my-export'),
    targetNodePath: '/content',
    nodeImported: (count) => log.info(`Imported: ${count}`),
    nodeResolved: (count) => log.info(`Resolved: ${count}`),
});
// result.skippedNodes did not exist
----
...
----
const result = importNodes({
    source: resolve('/import/my-export.zip'),
    targetNodePath: '/content',
    nodeImported: (count) => log.info(`Imported: ${count}`),
    nodeResolved: (count) => log.info(`Resolved: ${count}`),
    nodeSkipped: (count) => log.info(`Skipped: ${count}`),
});
// result.skippedNodes is now available
----
...
The `dryRun` field has been removed from the response.
...
----
{
  "sourceRepoPath": "myrepo:branch:/content",
  "exportName": "my-export",
  "exportWithIds": true,
  "includeVersions": false,
  "dryRun": false
...
The `dryRun` request parameter and `dryRun` response field have been removed.

The response now includes a `skippedNodes` array listing node paths that were skipped during import.

The `exportName` must refer to a ZIP archive present in the exports directory (`.zip` file).
Folder-based exports from XP 7 must be converted to ZIP format before they can be imported.

.XP 7 import request
[source,json]
----
{
  "targetRepoPath": "myrepo:branch:/content",
  "exportName": "my-export",
  "importWithIds": true,
  "importWithPermissions": true,
  "dryRun": false
}
----
...
.XP 7 import response
[source,json]
----
{
  "addedNodes": ["/content/mynode"],
  "updatedNodes": [],
  "importedBinaries": [],
  "importErrors": [],
  "dryRun": false
...
Dumps are now stored as ZIP archives named `{name}.zip` in the dumps directory,
replacing the previous folder-based structure.
...
NOTE: The request body is unchanged, but the output is now a ZIP archive (`my-dump.zip`) instead of a folder.

==== POST /system/load

A new `archive` boolean parameter has been added to indicate whether the dump is a ZIP archive or a folder.

Dumps created in XP 8 are ZIP archives and require `archive: true`.
Folder-based dumps from XP 7 can still be loaded by setting `archive: false`.
