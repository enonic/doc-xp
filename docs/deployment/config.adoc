...
All configuration files (both .cfg and .properties) use the link:https://docs.oracle.com/javase/6/docs/api/java/util/Properties.html#load%28java.io.Reader%29[Java properties] format.
...
It also requires granting certain permissions. You can find more information in the documentation of link:https://github.com/hazelcast/hazelcast-kubernetes/tree/1.5.x[Hazelcast Discovery Plugin for Kubernetes].
...
Possible classes:
+
*ConsoleAppender*: "ch.qos.logback.core.ConsoleAppender" link:https://logback.qos.ch/manual/appenders.html#ConsoleAppender[ConsoleAppender] +
*FileAppender*: "ch.qos.logback.core.FileAppender" link:https://logback.qos.ch/manual/appenders.html#FileAppender[FileAppender] +
*RollingFileAppender*: "ch.qos.logback.core.rolling.RollingFileAppender" link:https://logback.qos.ch/manual/appenders.html#RollingFileAppender[RollingFileAppender]
...
See: link:https://logback.qos.ch/manual/configuration.html#loggerElement[Logger], link:https://logback.qos.ch/manual/configuration.html#rootElement[Root] for additional iformation

Additional information can be found here: link:https://logback.qos.ch/manual/configuration.html[Logback documentation]
...
|required
|Cron value describes how often the described task should be run. See string link:https://www.unix.com/man-page/linux/5/crontab[format].

|`init-job.<my-job-name>.timezone`
|optional
|Timezone used for cron. See string link:https://docs.oracle.com/en/java/javase/11/docs/api/java[format]. JVM value will be used if nothing is set.
