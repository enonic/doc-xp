...
<configuration scan="true" scanPeriod="60 seconds"> <!--1-->
  ...
  <logger name="org.apache.ignite" additivity="false"> <--3-->
    <level value="WARN"/>
...
<3> *Logger* Configuring loggers possible.
<4> *root* Configuring the root logger. 
+
...
Additional information can be found here: link:http://logback.qos.ch/manual/configuration.html[logback]
