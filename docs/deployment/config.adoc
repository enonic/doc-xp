...
cache.capacity:: image:xp-7150.svg[XP 7.15.0,opts=inline]. Defines Heap memory limit for node version cache. The value is set in percentage of total Heap, or in memory units (`mb`, `gb`, ...). Default: `10%`
...
gzip.enabled:: Enables GZIP compression for responses. Default: `false`. Before image:xp-7150.svg[XP 7.15.0,opts=inline] the default was `true`.
