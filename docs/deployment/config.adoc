...
=== Hazelcast image:xp-740.svg[XP 7.4.0,opts=inline]
...
.Sample Hazelcast configuration for Kubernetes
[source,properties]
----
...
lightMember:: Hazelcast Lite member which does not own any partitions. Default: `false`.
...
network.join.kubernetes.serviceDns:: Service DNS for Kubernetes discovery.
...
network.interfaces.enabled:: Enables specification of which network interfaces that Hazelcast should use. Default: `false`.

network.interfaces:: Comma separated list of ip addresses for binding.
...
system.hazelcast.initial.min.cluster.size:: Initial expected cluster size to wait before the node to start completely. Default: `0`.
...
=== SessionStore image:xp-740.svg[XP 7.4.0,opts=inline]
...
storeMode:: Switches between SessionStore mode. Can be `replicated` or `non-persistent`. In Replicated mode sessions distributed over nodes in cluster using Jetty-Hazelcast integration. Default: `non-persistent`.
