...
site.preview.contentSecurityPolicy:: Defines default value for Content-Security-Policy header for pages. Applied in `preview`/`inline` mode and only if page controllers do not specify own Content-Security-Policy header.
...
auditlog.filter:: Comma separated values `rule(,rule)\*` of allow/deny rules. Default: `*` (allow all).
...
Rules applied in order. First matching rule wins. If no matching rule found, event type is denied.
