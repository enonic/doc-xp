...
.Deny all applicatrions (system and local apps still get installed)
[source,properties]
----
filter = !*
----

.Deniy snapshotter app to be installed (still possible to install locally), all other applications allowed.
[source,properties]
----
...
Rule can be prefixed with `!` - that it is a deny rule, otherwise it is allow rule.
...
NOTE: Filter rules apply separately on different nodes in cluster. If one cluster node denys an app, another cluster node may still allow it.
