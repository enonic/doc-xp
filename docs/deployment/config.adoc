...
page.previewContentSecurityPolicy:: Defines default value for Content-Security-Policy header for pages. Applied in `preview`/`inline` mode and only if page controllers do not specify own Content-Security-Policy header. Default: `default-src 'self'; object-src 'none'; img-src 'self' data:; style-src * 'unsafe-inline'; font-src * data:'`
