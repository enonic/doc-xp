...
page.defaultContentSecurityPolicy:: Defines default value for Content-Security-Policy header for pages. Is not applied in `live` mode or if page controllers specify own CSP header. Default: `default-src 'self'; object-src 'none'; style-src 'self' 'unsafe-inline'`
