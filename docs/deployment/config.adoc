...
init-job.my-job2.description=Job to run `myapp:task2` every minute with `config` params
...
`XP_HOME/config/com.enonic.xp.admin.rest`

Configure the contentstudio admin behavior.
...
contentTypePatternMode:: Defines matching mode for content type patterns Can be `MATCH` or `LEGACY`. Default: `MATCH`. See <<../cms/input-types#allowContentType, description>>.
