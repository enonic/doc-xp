...
network.publish.host:: sets the address other nodes will use to communicate with this node. Default is not set (before XP 7.5.0 `127.0.0.1`). Cannot be more than one IP-address. Can be an explicit IP-address, a host-name or an alias. See the section below for an overview of aliases.
