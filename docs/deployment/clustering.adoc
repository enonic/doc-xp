...
Since Elasticsearch is embedded in XP, you must also configure the following kernel settings on every XP node:
...
A malfunctioning node must be OOM-killed rather than allowed to swap, as swapping can cause severe performance degradation and mask memory issues.

.Allow more memory-mapped areas
[source,bash]
----
$ sysctl -w vm.max_map_count=262144
----

Elasticsearch requires many `mmap` calls and will fail to start if this limit is too low.

To make these settings permanent across reboots, add them to `/etc/sysctl.d/`:

[source,bash]
----
$ echo "vm.swappiness=1" >> /etc/sysctl.d/99-xp.conf
...
* Setup virtual memory and swap settings on servers correctly: set `vm.max_map_count=262144` and `vm.swappiness=1`.
