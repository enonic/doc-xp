...
A clustered deployment needs a load balancer in front of the nodes to distribute requests. Load balancing — including session affinity (session replication versus sticky sessions) and health checks — is covered in <<edge#_load_balancing, Web server, load balancing and CDN>>.
