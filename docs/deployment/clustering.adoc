...
We will set the bare minimum for those files. You can read the <<./../config/system#,Configuration>> documentation for more details.
...
For session-based logins, XP now supports https://developer.enonic.com/docs/xp/stable/config/system#sessionstore[session replication] between nodes. This effectively prevents users from loosing their session, even when a node is stopped, but this does not mitigate the problems mentioned above.
...
TIP: It is possible to free master nodes from running named tasks by setting `distributable.acceptInbound = false` in <<../deployment/config.adoc#_task, Task>> config.
