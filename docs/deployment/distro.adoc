...
== Downloading
Pre-compiled XP distributions can be manually downloaded from the https://repo.enonic.com/#browse/browse:public:com[Enonic Repo], or built from source on https://github.com/enonic/xp[GitHub].

Also check out the <<docker#XP Docker image>> versions of XP, which are based on the same distributions, but optimized for containerized deployment.
...
For development on the XP platform, check out the https://developer.enonic.com/docs/code[Enonic Developer Docs].
