...
Every time the Enonic XP image is built it will be based off the latest link:https://hub.docker.com/r/enonic/graalvm[enonic/graalvm] docker image. This ensures the latest updates to the JVM and OS.
...
app.sh:: Script to add/remove jars from the Enonic XP deploy folder. An example usage would be `app.sh add <PUBLIC_URL_TO_JAR>`. Description and source link:https://github.com/enonic/docker-xp7/blob/master/bin/app.sh[here].

backups.sh:: Convenient script for pre backup hooks to check the age of index snapshots created by the link:https://market.enonic.com/vendors/enonic/snapshotter[snapshotter] app. Description and source available https://github.com/enonic/docker-xp7/blob/master/bin/backup.sh[here].

dump.sh:: Easily create JVM heap and thread dump. Description and source available link:https://github.com/enonic/docker-xp7/blob/master/bin/dump.sh[here].

jattach:: A tiny tool to enable creating thread and heap dumps without the full JDK in the image. Description and source available link:https://github.com/apangin/jattach[here].
...
$XP_HOME/snapshots:: Index snapshots created by the link:https://market.enonic.com/vendors/enonic/snapshotter[snapshotter] app.
...
Since this image uses a modern Java VM, it will respect the container memory limits set by the docker daemon. There are some pitfalls though. Elasticsearch uses off-heap buffers, that can lead to the container running out of memory. For that reason, as a general rule, you should:
...
The source code for the `Dockerfile` and all related scripts is open source and can be viewed link:https://github.com/enonic/docker-xp7[here].
