...
== Useful tools
Enonic XP offers both <<../endpoints/management#, management endpoints>>, and internal APIs to manage your data.
...
If you for instance have performed a <<../storage/maintenance#vacuum, Vacuum>> operation that deletes blobs, restoring old snapshot will bring your solution into an inconsistent state. To prevent confusion, all snapshots are deleted after any vacuum operations that delete blobs.
====

[WARNING]
====
A blob-deleting vacuum is a point of no return for restores. All existing snapshots are deleted and the referenced blobs are removed, so you cannot restore to any moment before the vacuum ran — it becomes your new baseline.

Take a fresh blobstore backup and snapshot immediately afterwards. See <<../storage/maintenance#vacuum, Vacuuming>>.
====
...
. Backup the files in your <<../config/system#repo, snapshots folder>> to a safe location
. Backup the files in your <<../config/system#file-blobstore, blobstore folder>> to a safe location
...
. Copy the blobstore backup files into your instance's <<../config/system#file-blobstore, blobstore folder>>
. Copy the snapshot backup files into your instance's <<../config/system#repo, snapshots folder>>.
...
For ongoing operational maintenance — vacuuming unreferenced blobs and metadata, and rebuilding the search index — see <<../storage/maintenance#, Storage maintenance>>.
