...
`com.enonic.xp.web.security.cfg`

Security service configuration.

auditlog.enabled:: Set to `false` to disable the creation of audit log entries on security API operations such IAM and application changes. Default: `true`.

IMPORTANT: Audit log can be globally disabled in `<<logging#audit_enabled, Audit log config>>`.
...
auditlog.enabled:: Set to `false` to disable the creation of audit log entries on repository operations. Default: `true`.
IMPORTANT: Audit log can be disabled by the common `<<audit_enabled, enabled>>` property. Repository flag will not have any effect in this case.
...
baseDir:: specifies root of blobs. Default: `${xp.home}/repo/blob`.
...
`com.enonic.xp.api.cfg`

Manage settings for the Universal API

.Sample API configuration
[source,properties]
----
api.index.enabled=auto|on|off|
----

api.index.enabled:: Enable or disable the `/api` index page. Default: `auto` -> enabled if in dev mode, otherwise disabled.
...
NOTE: Exports dir should be a shared storage for cluster environments.