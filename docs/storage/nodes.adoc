...
A node essentially consists of:

* a set of *custom properties* — the actual data
* a set of *system properties* — metadata managed by XP (id, name, path, timestamps, etc.), prefixed with `_`
...
Properties hold the actual data inside a node. Each property is a key/value pair, where the value has a specific type — such as a string, number, date, or geographical point.

A simple node might carry properties like this:
...
Properties can also be nested using `.` as a path separator, and any value can be a list:
...
person.age = 39
person.birth-date = localDate("1975-10-17")
city.location = geoPoint("37.785146,-122.39758")
----

Here `person` and `city` act as containers (called _sets_) that group related properties together.
...
You can flag repository as _transient_, meaning that their data is not stored in system dumps and snapshots.
