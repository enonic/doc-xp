...
|string
|true
|A search string to match field value(s).
Support the set of <<search_string_operators,operators>>.
...
!===
!`OR` !Any of the words in the search-string matches (by default).
!`AND`!All words in search-string matches.
!===

|===
[#search_string_operators]
...
field/fields:: Comma-separated list of <<propertyPath, propertyPaths>> to include in the search.
...
NOTE: query filtering can be used together with separate <<filters#, filters>>.
