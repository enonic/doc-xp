...
XP enforces access control on every <<nodes#, node>> in the data store, all the way down to individual documents.
...
A node's ACL is shared across every <<branches#, branch>> the node exists in. Setting or applying permissions on a node takes effect immediately on that node in every branch — there is no per-branch ACL state and no "publish permissions" step between branches.
