...
range (from: number, to: number):: Defines a range to create a bucket for. `from` value will be included in the bucket, `to` will be excluded.
...
ranges (range[]):: The range-buckets to create.
range (from: <number>, to: <number>):: Defines a range to create a bucket for. `from` value will be included in the bucket, `to` will be excluded. Values in `from` and `to` follow a special date-math expression explained below.
