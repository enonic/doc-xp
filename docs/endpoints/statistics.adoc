...
NOTE: <<../config/vhosts#,Vhost>> configuration only applies to the web endpoint (port 8080) and does not affect the Statistics API.

== Monitoring

For operational purposes, Enonic XP provide advanced metrics for monitoring which are easily accessed. To access the monitoring JSON feed you can point to the following url:
