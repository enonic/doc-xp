...
The Web endpoint consists of four main services: <<#api, /api>>, <<#admin, /admin>>, <<#site, /site>> and <<#webapp, /webapp>>, each serving specific purposes and use cases.
...
NOTE: APIs can also be contextually mounted across other Web services via the `_` path:
...
Authentication is configured per vhost by mapping ID providers, so different parts of your platform can use different authentication mechanisms. See <<../config/vhosts#,Virtual Hosts>> for the mapping syntax and <<../security/idproviders#,ID providers>> for the available providers.

For service-to-service access, the platform supports JWT authentication via the system ID provider — see <<../security/service-accounts#jwt-authentication,JWT authentication>>. Individual endpoints may document additional authentication options; see for example <<management#authorization,Management API>>.
