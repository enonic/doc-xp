...
XP registers successfully completed actions that affect repositories, content, applications, principals and ID providers in the `system.auditlog` repository.
...
In order to enable or disable audit logs for repository actions you have to modify configuration file `com.enonic.xp.repo.cfg` and set `auditlog.enabled` option to `true` to enable or `false` to disable logging. Default value is `true`.
...
=== XP registers the following actions for repositories:

* `system.repo.create` - a repository has been created
* `system.repo.delete` - a repository has been deleted
* `system.repo.branch.create` - a branch has been created in a repository
* `system.repo.branch.delete` - a branch has been deleted from a repository
