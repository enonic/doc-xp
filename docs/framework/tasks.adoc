...
NOTE: For fine-grained control which nodes should be capable to execute distributable tasks use <<../deployment/config.adoc#_application, Application>> filter config or local apps. Or disable execution of named tasks entirely on some nodes by <<../deployment/config.adoc#_task, Task>> config.
