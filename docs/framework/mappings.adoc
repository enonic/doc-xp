...
NOTE: Mappings are only available in the <<../runtime/engines/site-engine#, Site Engine>>

== Introduction

Mappings is the first processing step in the <<../runtime/engines/site-engine#, site engine>> - before page rendering starts.

You may define mappings for both <<controllers#, controllers>> and <<filters#,filters>>. The use cases are vast. 

== Usage

Mappings are defined in the <<../cms/sites#site_xml, site descriptor>>.
...
the mapping with the *lower* `order` value (as defined in the <<../cms/sites#site_xml, site descriptor>>) is used.

In case there are several mappings with the same ``order`` number,
the position of the applications (as configured within the site) determines which mapping will be used.
