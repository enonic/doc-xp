...
<6> *@service* specifies a system service which will be handled by the filter i.e. (`.../_/media:image/...` endpoint).
...
Allows to filter or redirect to a controller rendering of images on URLs like `<host>:<port>/site/<repo>/<branch>/<path-to-content>/_/media:image/<image-path>`
