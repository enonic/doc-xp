...
Site Mappings now allow to customize the behavior of Image, Attachment and Component <<cms/mappings#services, Services>>.

=== Project Applications

<<cms/projects#applications, Project Applications>> feature is now production ready and turned on by default.

=== Virtual Applications and No-code Schema

<<apps/virtual-applications#virtual_applications, Virtual Applications>> feature was introduced as an experimental feature in 7.11.
As of 7.12 this feature is still experimental but has been refactored and improved:
...
* Site <<cms/mappings#mappings, Mappings>> work with Projects as well
...
* Restricted access to draft branch. Can be configured via `draftBranchAllowedFor` setting in the <<deployment/config#portal, portal config>>
