...
<<cms/projects#_applications, Project Applications>> feature was introduced as an experimental feature in 7.11 and is now officially announced in 7.12.
Since the previous release this feature has been refactored and improved:

* Application configurations are now stored in the repository itself, similar to how application configurations are stored on a site.
* Site <<cms/mappings#_mappings, Mappings>> work with Projects as well

=== Other improvements

* Virtual Applications and No-code Schemas are no longer experimental features. Read more about <<apps/virtual-applications#_virtual_applications, Virtual Applications>>.
...
* Restricted access to draft branch. Can be configured via `draftBranchAllowedFor` setting in the <<deployment/config#_portal, portal config>>
