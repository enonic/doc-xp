...
Say goodbye to passwords. The built-in <<iam#system-id-provider, System ID provider>> has been upgraded to support use of tokens.
Users created created in the System ID provider are now referred to as <<iam#service-accounts, Service Accounts>>.
...
NOTE: If you are using automated appl deployments we recommend switching to token based authentication.

=== Progressive image delivery

The image service now supports progressive image formats. This means images images are loaded in multiple passes, rather than serially - which may improve perceived performance.
