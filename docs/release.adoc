...
Content Layers allow to automatically replicate content between dependant repositories.
This feature can be used to implement multi language sites.
JavaScript API is also available.
...
* Management Port got extended to be able to start, stop and uninstall applications.

* A new SSE endpoint allows to list installed applications and listen for application status changes.
