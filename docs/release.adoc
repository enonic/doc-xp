...
a very important architectural change primarily made to make clustered installations more stable.

NOTE: Please read the <<release/upgrade#, upgrade notes>> in relation to backward compatibility.

=== Hazelcast
Events, tasks and sessions are done with Hazelcast instead of ElasticSearch.
This is a big archtectural change that improves the behaviour of clustered installations of XP.
With Hazelcast, XP also gets the support of distributed sessions.

=== Other improvements

The following improvements are also part of this release:

* Calculated distances in geoDistance aggregations are exposed.
* `AlwaysPreTouch` is enabled to avoid extra GC pauses.
