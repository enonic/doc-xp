...
We recommend bundling Gradle wrapper, version 5 in your project
...
You will also need to install the 2.0 version of the XP Gradle plugin.
This supports Gradle 5, Java 11 and building for XP 7.
...
Additionally, this is the recommended format for settings.gradle
...
NOTE: appName and appDisplayName are only used for application projects.
...
For all schemas in XP 6, the input types <<../reference/cms/schemas/input-types#contentSelector,contentSelector>> and <<../reference/cms/schemas/input-types#tag,tag>> defaulted to selecting content across the entire repository.
...
The old <display-name-script> functionality is deprecated, and replaced by a ES 6 expressions (TODO)
...
* <allowContentType> config is moved to site.xml
...
----
<site>
  <x-data name="my-sample" allowContentType=".*:folder">
</site>
----
...
Image styles also introduces the ability to use image service filters, and client side CSS styles that are used by the editor.

Read more about Image styles (TODO, link)


=== Inline scripts

XP 6 allows editors to add script tags to the rich text editor source code.
XP 7 automatically removes such scripts when data are stored.
This measurement effectively reduces the attack surface of Enonic XP.

Consider using different solutions for advanced functionality in the editor, such as macros.
...
Update your project structure by renaming the filter folder from `/src/main/resources/site/filters` to ``/src/main/resources/site/processors`
...
Until now, the render modes have been: ``PREVIEW``, ``EDIT`` and "TODO"
TODO: Verify last mode

With XP 7, the `ÌNLINE` render mode is introduced.
...
With XP 7, a dedicated web app controller /src/main/resources/webapp/webapp.js is introduced.
...
Also, please check out the documentation of the <<../reference/runtime/framework#Require(),require function>>.
...
As such, a widget must be able to determine which repository it should connect to.

CMS repos are consistently named as
...
Where XP 6 depends on Java 8, XP 7 bundles Java 11.
