...
The schema system provides a range of standard <<cms/schemas#input_types,Input types>>, such as TextLine and ContentSelector.
...
The XP Site engine is based on the <<./runtime#webapp-engine,XP Web engine>>.
...
Developers can still take control over URLs when required, through the use of <<./framework/http#http-filter,Filters>> and <<./cms/mappings#,Mappings>>.
