...
The schema system provides a range of standard link:./cms/schemas#input_types[Input types], such as TextLine and ContentSelector.
...
The XP Site engine is based on the link:./runtime#webapp-engine[XP Web engine].
As such, it also uses the <<./framework#,Javascript Framework>>, but extends the use.
...
Developers can still take control over URLs when required, through the use of link:./framework/http#http-filter[Filters] and <<./cms/mappings#,Mappings>>.
...
Enonic XP contains an link:./runtime/image-service[Image Service] that can scale, process and deliver rasterized RGB/RGBa image media in real-time.
