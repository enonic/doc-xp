...
Mapping are defined in the <<sites#site.xml, site descriptor>>.
...
<1> *@controller* defines the path to a standard <<../framework/http#HTTP Controller, HTTP controller>>
...
<5> *@filter* defines the path to the <<../framework/http#HTTP Filter, HTTP filter>>
...
the mapping with the *lower* `order` value (as defined in the <<sites#site.xml, site descriptor>>) is used.
