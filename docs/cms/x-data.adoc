...
If `allowContentTypes` and `optional` attributes are not set, the x-data will automatically be enabled for all content types. See <<input-types/contentselector#allowContentType, modes>> of content type filtering.
