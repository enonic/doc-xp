...
_@i18n_ is an optional attribute holding the key to localization phrase of the form (see <<schemas#translations,a mixin>>)
...
== Sets
In order to build richer forms that support grouping and nesting, Enonic XP supports the following input types:
...
To simplify maintenance of forms, mixins offers a simple templating approach to reuse the same definition across various forms.
A mixin or x-data can be injected into a form simply by referencing it by name.

.Sample mixin "us-address"
[source,xml]
----
<mixin>
...
<content-type>
  ...
  <form>
    ... fields specific for this content type ...
    <mixin name="us-address"/> <!--1-->
  </form>
</content-type>
----

<1> The `<mixin>` element will be replaced by the contents of the `<form>` element from the mixin schema.

.Content Type using the "us-address" X-data
[source,xml]
----
<content-type>
  <display-name>Using mixins</display-name>
  ...
  <x-data name="us-address"/> <!--1-->
</content-type>
----

<1> The `<x-data>` element will be replaced by the `<form>` element from the x-data schema.

Similar functionality can also be solved by compile-time build scripts if desired.


== Schema Localization

Based on the same pattern as described in <<../../app-engine/i18n#,Application localization>> one may also localize schemas.
This enables labels and texts in schemas be displayed in the preferred language according to the user's browser settings.
...
For more details, check out the <<../runtime/18n#,runtime localization docs>>
