...
Option sets allow editors to select the content of an instance between pre-defined options.
...
Each option may define zero or more items. Hence, a single option may be represented by the option alone, or provide a new set of inputs and sets - just like an item set.

By default, an option form will only be displayed upon selection of the option, but a multi-select option set can be configured to have all options expanded by default.

It's also possible to pre-select specific options by default.


.single-select option set for creating blocks of content
[source,xml]
----
...
.multi-select option set with options expanded by default, empty first option and pre-selected second option:
[source,xml]
----
