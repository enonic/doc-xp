...
TIP: image:xp-760.svg[XP 7.6.0,opts=inline] It's possible to configure a layout to be used only on specific content types by using the config `<allow-on-content-type>`:

.Example layout descriptor allowed for one content type
[source,XML]
----
<layout>
  <display-name>70/30</display-name>
...
.Example layout descriptor NOT allowed for content types that end with ":post"
[source,XML]
----
<layout>
  <display-name>70/30</display-name>
...
In this case the layout will be available only for the content type called "article" from the same application.
...
TIP: image:xp-760.svg[XP 7.6.0,opts=inline] Just like layouts, parts can be configured to be available only for specific content types by using the config `<allow-on-content-type>`.
