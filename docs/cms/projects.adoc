...
Just like with sites, applications can be added to projects. All functionality and all schemas (content types, schemas, mixins) and descriptors (pages, parts, layouts) from the app will be available within the scope of the project down to the site. Sites override project applications. If a site needs an application from its project - add the application to the site.
