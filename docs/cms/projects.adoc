...
Just like with sites, applications can be added to projects and config could be specified for every application. All functionality and all schemas (content types, schemas, mixins) and descriptors (pages, parts, layouts) from an app will be available within the scope of the project down to a site.
