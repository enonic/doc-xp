...
NOTE: Prior to XP image:xp-7150.svg[XP 7.15.0,opts=inline], manually added users to the System ID provider were treated as regular users with username-password credentials. System ID provider username-password credentials are still supported, but strongly discouraged. Instead, use service accounts with JWT tokens. See <<#service-accounts>> for more details.
...
Each Service Account may (but not required to) have a Service Account Key. Or multiple service account keys.
...
Service accounts keys are used to make authorized call to:
