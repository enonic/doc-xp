...
*Array.<object>* : A list of JSON objects for each member of the principal or empty array if none.
...
*object* : The principal as a JSON object

.Retrieving the full principal for 'user1'
[source,js]
----
var result = authLib.getPrincipal('user:enonic:user1');
----
...
| scope | string | Scope of the data to retrieve.
|===

[.lead]
Returns

*object* : Profile data as JSON
...
//[source,js]
//----
//var result = authLib.getProfile('user:enonic:user1');
//----
//
//.Sample response
//[source,js]
...
*object* : User data in JSON object
