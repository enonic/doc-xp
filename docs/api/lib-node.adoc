...
image:xp-7120.svg[XP 7.12.0,opts=inline] This function duplicates a node.
...
=== applyPermissions
image:xp-80.svg[XP 8.0,opts=inline]
...
| scope | string ('SINGLE' \| 'TREE' \| 'CHILDREN') | <optional>  | scope of applying permissions. Scope 'SINGLE' applies permissions only to the node itself. 'TREE' applies permissions to the node and its descendants. 'CHILDREN' applies permissions only to the node's descendants. Default is 'SINGLE'.
