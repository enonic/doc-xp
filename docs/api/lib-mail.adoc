...
The `from` parameter supports the <<getDefaultFromEmail,default _from_ email>>.
If the email address is not specified in brackets (e.g., `Some Name <name@domain.org>` or `<>`), the default _from_ email address is used. 
...
image:xp-7141.svg[XP 7.14.1,opts=inline] This function returns the default _from_ email address configured in the XP mail configuration.
