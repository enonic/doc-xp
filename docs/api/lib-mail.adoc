...
This library includes a function for sending emails, as well as a function for retrieving the `defaultFromEmail` address from the configuration.
...
image:xp-7141.svg[XP 7.14.1,opts=inline] This function returns the default from email address configured in the XP mail configuration.
