...
| focalX | number | <optional> | | Focal point for X axis (if it's an image)
| focalY | number | <optional> | | Focal point for Y axis (if it's an image)
| caption | string | <optional> | | Caption to the content
| artist | string \| Array.<string>  | <optional> | | Artist to the content
| copyright | string | <optional> | | Copyright to the content
| tags | string \| Array.<string> | <optional> | | Tags to the content
| workflowInfo | object | <optional> | | Workflow information to use. Default has state READY and empty check list.
