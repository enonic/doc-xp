...
| version | string | <optional> | | Content version id
|===

NOTE: `version` was first introduced in v7.2.0
...
image:xp-750.svg[XP 7.5.0,opts=inline] If sort specified for a query, results will contain a meta system properties `_sort` and `_score: null`, otherwise `_score`
 will has a relevant value.
