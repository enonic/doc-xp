...
if (result) {
    log.info('Content permissions: ' + JSON.stringify(result));
} else {
...
NOTE: If the content is published, the permissions will be applied to the content there as well without a need to republish content.
[.lead]
*Parameters:*
...
| scope | string ('SINGLE' \| 'TREE' \| 'CHILDREN') | <optional>  | scope of applying permissions. Scope 'SINGLE' applies permissions only to the content itself. 'TREE' applies permissions to the content and its descendants. 'CHILDREN' applies permissions only to the content's descendants. Default is 'SINGLE'.
|===
