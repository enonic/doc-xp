...
TIP: It is possible to export all nodes in the repository's branch by setting `sourceNodePath` to `/`.
...
NOTE: It is possible to import the whole repository's branch by setting `targetNodePath` to `/`, but only if export is made for the whole branch as well.
