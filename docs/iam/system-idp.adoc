...
IMPORTANT: Your _Private_ key from the Public-Private key pair is stored only on the client side and must never be shared with the XP server.
The _Public_ key from Public-Private key pair is stored in XP, and is used to verify the signature of your token.
...
A JWT token must be a valid https://datatracker.ietf.org/doc/html/rfc7519[RFC-7519] JWT token, following these requirements:
...
JWT parts must be signed and encoded according to https://datatracker.ietf.org/doc/html/rfc7519[RFC-7519].
