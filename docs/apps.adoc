...
Via API:: Virtual applications can be created with <<api/lib-app, applications API>>. They allow to extend/override schemas and descriptors from usual bundled application or create separated containers for it. Priority for virtual applications can be set in <<deployment/config#application, app config>>.
